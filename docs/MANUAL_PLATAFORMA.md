...
**Versão:** 1.8.1 — auditada contra o código
**Data:** agosto/2026  
...
**Saudação que anuncia a fila errada** (`QueueGreetingConsistency.ts`). A escolha do cliente não se desalinha sozinha: `verifyQueue` indexa o mesmo array que exibiu, então `queues[3-1]` é sempre a terceira opção mostrada. O que se desalinha é o **texto** — cada saudação é escrita à mão, e copiar a de uma fila para outra faz o cliente escolher "Recuperar Conta" e ler "Você foi direcionado ao Suporte Empresa". O ticket vai para a fila certa; só a mensagem mente, e por isso o sintoma parece bug de roteamento e não aparece em nenhuma outra checagem.

`detectQueueGreetingMismatches` compara o núcleo do nome de cada fila (sem o prefixo `01 - `) com a própria saudação: se a saudação nomeia **outra** fila e não a sua, `AuditSupportLinesService` emite `queue_greeting_announces_other_queue` como **warn** — o atendimento funciona, o texto é que está errado. Nome curto demais (`Suporte`, `Vendas`) não entra no critério, para não gerar alarme falso, e citar outra fila de passagem não conta se a própria estiver nomeada.
