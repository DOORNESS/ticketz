...
export async function up(queryInterface: QueryInterface) {
  await queryInterface.sequelize.transaction(async transaction => {
    ...
    await queryInterface.sequelize.query(
      `
      CREATE OR REPLACE FUNCTION ticketz.ticketz_safe_jsonb_baileyscontacts_20260312(input_text TEXT)
      RETURNS JSONB
      LANGUAGE plpgsql
      AS $$
      DECLARE
    ...
    await queryInterface.sequelize.query(
      ...
      WITH extracted AS (
        ...
        CROSS JOIN LATERAL jsonb_array_elements(
          ticketz.ticketz_safe_jsonb_baileyscontacts_20260312(b."contacts")
        ) elem
    ...
    await queryInterface.sequelize.query(
      `
      DROP FUNCTION IF EXISTS ticketz.ticketz_safe_jsonb_baileyscontacts_20260312(TEXT);
    `,
      { transaction }
    );
