...
describe("12-13. marca nova não exige condicional no código", () => {
  ...
  /**
   * FortControl é produto da suíte Fortmax (PCP, estoque, financeiro), não uma
   * operação com atendimento próprio — comprovado ao varrer o código: aparece
   * só como assunto dentro do conhecimento Fortmax. Estes casos travam essa
   * decisão para que ninguém "prepare" uma marca vazia por engano.
   */
  describe("FortControl pertence à Fortmax", () => {
    it("conexão que cita FortControl é da Fortmax", () => {
      expect(legacyMatchBrandSlugByName("Fortmax FortControl")).toBe("fortmax");
      expect(legacyMatchBrandSlugByName("FortControl Suporte")).toBe("fortmax");
    });

    it("conexão Fortmax sem citar FortControl segue igual", () => {
      expect(legacyMatchBrandSlugByName("Suporte Fortmax WebG3")).toBe(
        "fortmax"
      );
    });

    it("uma marca futura vem da FK, não de nome novo no código", async () => {
      // Nenhum padrão casa, e ainda assim a marca criada pelo painel resolve.
      expect(legacyMatchBrandSlugByName("Canal FortControl 2027")).toBe(
        "fortmax"
      );
      (Whatsapp.findOne as jest.Mock).mockResolvedValue({
        id: 77,
        name: "Canal FortControl 2027",
        brandId: 44,
        companyId: 1
      });
      ...
      await expect(resolveBrandIdForWhatsapp(1, 77)).resolves.toBe(44);
    });
  });
});
